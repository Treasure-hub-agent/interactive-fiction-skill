...
| 文件 | 用途 | 结构与约束 |
|------|------|-----------|
| `tags.json` | 选项策略标签词库 | `general[7]`（type/pace/tags[5]）+ `intimacy[7]`（tag/pace）。约束：general 内 35 词全库去重；pace 取值 `快/慢/迂回`（单值）；A/B/C 选词须不同类型（互斥）且覆盖快/慢/迂回三节奏；亲密场景用 intimacy 池 |
...
数据层的 `pace` 字段仅允许 `快 / 慢 / 迂回` 三个单值（亲密池与通用池同规则）。开发期可用脚本校验全部 pace 合法、标签无重复（见 MANIFEST 生成流程的配套校验思路）。
