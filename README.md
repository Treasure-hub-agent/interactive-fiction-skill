...
> 本栏目记录**当前版本**的重要变动，每次新版本更新一次、仅保留最新版本；完整历史见 `references/changelog.md`。

**🎭 蒸馏角色功能强化（重点）**
- 角色卡新增「存在形态」维度（人类 / 非人 / 化身 / 组织 / 多身份）——神祇、机械生命、妖兽、组织化身、多身份间谍等特殊角色全覆盖
- 强制「信息充足度评估」（五维 高/中/低 + `[待补]`，低维度禁编造）
- 冷门配角「轻量卡 + 渐进补全」路径——信息不足不硬凑五维，可随时升级完整卡

**🗃️ 数据层外置（实验性）**
- 标签词库 / 字数区间 / 指令路由 / 随机池 固化为 `data/*.json`，SKILL.md P0 瘦身、按需读取
- `schema/novel_runtime.schema.json` 运行时格式正式化（含 choices[] 升级字段）

**🔄 叙事与体验**
- 自由输入优先 + 记忆核对三查（设定 / 后果 / 回响），自由行为 / 台词正式记账
- 随机开局池数据化 + 换批去重拒绝采样
- 亲密池 7 项（A/B/C 三节奏组合提升 4 倍）
- 双玩家实测 + 8 项专项实测驱动，修复 19+ 项规范问题

---

## ✨ SKILL 功能快速简介

**interactive-fiction** 是一个 AI agent 互动小说创作规范（skill）：让 AI 写出「有选项、有张力、有沉浸感」的互动小说，一个 skill 达到酒馆（SillyTavern）级体验。

| 能力 | 说明 |
|------|------|
| 🎭 选项驱动 | 每轮正文必跟 A/B/C/D/E 选项（参考指引 + 兜底），自由输入等权/优先推进 |
| 📖 三种模式 | 主线 / 沉浸 / 爽文，随时切换 |
| 🎴 角色卡体系 | 蒸馏（含轻量卡）/ 深度创建，九区块人设 + 存在形态 |
| 💾 存档系统 | 轻量 / 持久 / 里程碑三档，断连恢复、切换小说、导出 |
| 🔍 记忆核对 | 设定 / 后果 / 回响三查，长剧情零漂移 |
| 🗃️ 数据层外置 | 查表规则固化为 `data/*.json`（v9.4.0），AI 按需读取 |
