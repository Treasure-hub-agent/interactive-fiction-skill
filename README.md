...
**interactive-fiction** 是给 AI 用的「互动小说创作指南」——装上它，AI 就能写出带选项、带剧情的互动小说，像玩文字冒险游戏一样，体验不输专业小说软件（如酒馆 SillyTavern）。
...
**🎭 角色蒸馏大幅强化（本次重点）**
- 能建的角色更多了：神祇、机器人、妖兽、组织化身、双面间谍等特殊角色都能还原成角色卡
- 资料少的角色也有办法：信息不足时 AI 会诚实标注「待补充」，绝不瞎编；冷门小配角可以先用「轻量卡」快速建档，之后随时补全升级成完整角色

**⚡ 运行更轻快（实验性）**
- 常用规则（词汇库、字数、指令、随机池）整理成数据文件，AI 按需读取，加载更快、更稳

**🔄 玩起来更顺手**
- 自由输入更聪明：你直接说的话、做的事会正式记入剧情，AI 记得住、接得上
- 随机开局更丰富：连抽几次不会重复，角色模板更均匀
- 亲密场景选择更多：快 / 慢 / 迂回三种节奏的组合提升 4 倍
- 细节打磨：修复 19+ 项实测发现的问题，体验更顺滑
